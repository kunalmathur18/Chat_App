...
//     const { chatId, user } = useChatStore();  // Removed unused isCurrentUserBlocked and isReceiverBlocked
//     const { currentUser } = useUserStore();
//     const [chatUser, setChatUser] = useState(user); // Adding chatUser state

//     const endRef = useRef(null);

//     // Scroll to the last message on each render
//     useEffect(() => {
//         endRef.current?.scrollIntoView({ behavior: "smooth" });
//     }, [chat?.messages]);

//     // Listen for updates to the chat user's avatar and bio
//     useEffect(() => {
//         if (user?.id) {
...
//                     />
//                     <div className="text flex flex-col gap-2">
//                         <span className='text-[22px] text-[#f49778] font-bold'>{chatUser?.username}</span>
//                         <p className='text-[18px] font-light text-[#f0e9e9]'>
...
//                     onChange={(e) => setText(e.target.value)}
//                     value={text}
//                     disabled={false} // Keep default behavior for blocking
//                 />
//                 <div className="emoji relative">
//                     <MdEmojiEmotions className='cursor-pointer text-[20px]' onClick={() => setOpen((prev) => !prev)} />
//                     <div className="picker absolute bottom-[50px] left-0 z-[999]">
