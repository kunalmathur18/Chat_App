...
///testing below it is working good and above also
...
//     // Real-time updates for avatar and bio
//     useEffect(() => {
//         if (user?.id) {
...
//                     setAvatarPreview(data?.avatar || avatar); // Update avatar if exists
//                 }
//             });
...
//                         </div>
//                     ) : (
//                         <p>{bio}</p> // Display the real-time updated bio
//                     )}
...
function Details({ onBack }) {  // Added onBack prop
    const { chatId, user, isCurrentUserBlocked, isReceiverBlocked, changeBlock } = useChatStore();
    const { currentUser, fetchUserInfo } = useUserStore();
    const [bio, setBio] = useState(user?.bio || "No bio available");  // Initial bio
    const [isEditing, setIsEditing] = useState(false);  // Toggle edit mode
    ...
    return (
        <div className='details bg-[rgba(0,0,0,0.41)] rounded-[16px] shadow-[0_4px_30px_rgba(0,0,0,0.1)] backdrop-blur-[10.7px] border-[1px] border-solid border-[rgba(0,0,0,1)] p-[10px]'>
            <div className='w-[200px]'>
                <div className="user flex py-[20px] px-[20px] flex-col items-center gap-[10px] border-b-[1px] border-solid border-[#dddddd35]">
                    <div className='flex gap-2 text-white'>
                        <FaArrowLeft onClick={onBack} className="cursor-pointer" />  {/* Call onBack function when arrow is clicked */}
                        <img
                            className='w-[100px] h-[100px] rounded-full object-cover cursor-pointer'
                            src={avatarPreview} 
                            alt=""
                            onClick={() => setIsAvatarEditing(true)}  // Open avatar editing mode
                        />
                    </div>
